...
### 构建TensorRT （仅英伟达用户）
若是英伟达用户，在安装并确认上一步没问题后可以点击`01C.构建TensorRT加速（仅英伟达显卡）.bat`，这将会需要较长时间对模型进行TensorRT编译（>30min）。若这一步报错，有三种可能  
...
点击`01C.构建TensorRT加速（仅英伟达显卡）.bat`构建TensorRT
...
## 输入输出设备  
#### SPOUT2 输出
目前推荐这个方案使用透明通道最简单。请移步 https://github.com/Off-World-Live/obs-spout2-plugin  安装obs插件。
